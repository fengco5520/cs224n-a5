...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        char_list = self.words2charindices(sents) # convert sents to list of charId
        char_padded_list = pad_sents_char(char_list,self.char_pad) #(batch_size, max_sentence_length, max_word_length)
        sents_var = torch.tensor(char_padded_list, dtype = torch.long, device = device) # make it into tensor
        return sents_var.permute(1,0,2) #(max_sentence_length,batch_size, max_word_length)
