...
def question_1f_sanity_check():
    ...
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1f: Highway function")
    print ("-"*80)
    dropout_rate = 0.4
    x_conv_out =  torch.randn(BATCH_SIZE, EMBED_SIZE)
    highway = Highway(EMBED_SIZE,dropout_rate)
    highway_out = highway(x_conv_out)  # model(input) will automatically call forward function
    output_highway_out_size  = [BATCH_SIZE,EMBED_SIZE]

    assert list(highway_out.size()) == output_highway_out_size
...
def question_1g_sanity_check():
    ...
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1g: CNN function")
    print ("-"*80)
    padding = 1
    kernel_size = 5
    char_embed_size = 10
    max_word_size = 6
    num_filter = 7
    cnn = CNN(max_word_size = max_word_size, char_embed_size = char_embed_size, 
        num_filter = num_filter, kernel_size =kernel_size, padding_size = 1)
    # initiate a dummy tensor (batch_size, char_embed_size, word_size)
    x_reshape = torch.randn(BATCH_SIZE, char_embed_size, max_word_size)
    x_conv_out = cnn(x_reshape)

    output_conv_out_size = [BATCH_SIZE,num_filter]

    assert list(x_conv_out.size()) == output_conv_out_size

    print("Sanity Check Passed for Question 1f: CNN function!")
    print("-"*80)
